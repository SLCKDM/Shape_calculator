...
class UnitTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.cylinder = Cylinder(3, 5)
        self.cone = Cone(4, 8)
        self.volumetric_examples = [self.sphere, self.cube, self.parallelepiped,
                                    self.pyramid, self.cylinder, self.cone]
    ...
    def test_subclass(self):
        ...
        for shape in volumetric_shapes:
            ...
            self.assertEqual(
                actual, expected,
                f'Expected {shape.__name__} class to be a subclass of the Volumetris class.'
            )

    def test_distinct_classes(self):
        """ Checking if the subclass of the shape is correct """
        for shape in volumetric_shapes:
            actual = shape is not Flat
            expected = True
            self.assertEqual(
                actual, expected,
                f'Expected {shape.__name__} class to be a distinct class from the Flat class.'
            )
    ...
    def test_volume(self):
        ...
        for ind, shape in enumerate(self.volumetric_examples):
            self.assertEqual(shape.volume(), res[ind],
                             f'{shape._title} should be {res[ind]}, got {shape.volume()}')
    ...
    def test_sphere_diameter(self):
        ...
        self.assertEqual(shape.diameter(), res,
                         f'{shape._title} should be {res}, got {shape.diameter()}')
    ...
    def test_diagonale(self):
        ...
        for ind, shape in enumerate(shapes):
            self.assertEqual(shape.diagonal(), res[ind],
                             f'{shape._title} should be {res[ind]}, got {shape.diagonal()}')
    ...
    def test_apothem(self):
        ...
        self.assertEqual(shape.apothem, res,
                         f'{shape._title} should be {res}, got {shape.apothem}')
    ...
    def test_area_side_cylinder(self):
        ...
        self.assertEqual(shape.area(True), res,
                         f'{shape._title} should be {res}, got {shape.area(True)}')
