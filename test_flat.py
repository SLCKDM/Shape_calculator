...
class UnitTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.trapezoid = Trapezoid(4, 5, 4, 15, 4.5)
        self.triangle = Triangle(4, 4, 3)
    ...
    def test_subclass(self):
        ...
        for shape in flat_shapes:
            ...
            self.assertEqual(
                actual, expected,
                f'Expected {shape.__name__} class to be a subclass of the Flat class.'
            )

    def test_distinct_classes(self):
        """ Checking if the subclass of the shape is correct """
        for shape in flat_shapes:
            actual = shape is not Volumetric
            expected = True
            self.assertEqual(
                actual, expected,
                f'Expected {shape.__name__} class to be a distinct class from the Volumeric class.'
            )
    ...
    def test_perimeter(self):
        ...
        for ind, shape in enumerate(self.flat_examples):
            self.assertEqual(shape.perimeter(), res[ind],
                             f'{shape._title} should be {res[ind]}, got {shape.area()}')
    ...
    def test_diagonal(self):
        ...
        for i, shape in enumerate(shapes):
            self.assertEqual(shape.diagonal(), res[i],
                             f'{shape._title} should be {res[i]}, got {shape.diagonal()}')
    ...
    def test_mid_line(self):
        ...
        self.assertEqual(shape.mid_line(), res,
                         f'{shape._title} should be {res}, got {shape.mid_line()}')
    ...
    def test_triangle_third_height(self):
        ...
        self.assertEqual(shape.height, 3.7080992435478315,
                         f'{shape._title} should be {res}, got {shape.height}')
